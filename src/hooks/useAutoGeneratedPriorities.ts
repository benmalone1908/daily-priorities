...
export function useAutoGeneratedPriorities(
  ...
  const { data: contractTerms = [], error: contractTermsError } = useQuery<ContractTerms[]>({
    queryKey: ['contract-terms-all'],
    queryFn: async () => {
      console.log('🔍 Fetching contract terms...');
      const { data, error } = await supabase
        .from('contract_terms')
        .select('*');

      if (error) {
        console.error('❌ Contract terms fetch error:', error);
        throw error;
      }
      console.log('✅ Contract terms fetched:', data?.length || 0, 'records');
      return data as ContractTerms[];
    },
    enabled: !!supabase
  });

  // Log contract terms query status
  if (contractTermsError) {
    console.error('❌ Contract terms query error:', contractTermsError);
  }
  ...
  const autoSections = useMemo(() => {
    ...
    console.log('🔍 ROAS Debug: Starting ROAS calculations');
    console.log('🔍 ROAS Debug: today =', today);
    console.log('🔍 ROAS Debug: Total campaigns in campaignMap:', campaignMap.size);
    console.log('🔍 ROAS Debug: Total contract terms:', contractTerms.length);
    ...
    campaignMap.forEach((rows, campaignName) => {
      ...
      if (totalSpend > 0) {
        ...
        if (roas < 2.0 && totalSpend > 100) { // Only flag campaigns with meaningful spend
          ...
          if (!matchingContract) {
            debugCounts.noContract++;
            console.log(`🔍 ROAS Debug: ❌ No contract for "${campaignName}" (ROAS: ${roas.toFixed(2)}, Spend: $${totalSpend.toFixed(2)})`);
            return;
          }
          ...
          // Skip campaigns with zero or negative days left
          if (daysLeft <= 0) {
            debugCounts.daysLeftZero++;
            console.log(`🔍 ROAS Debug: ❌ Days left is ${daysLeft} for "${campaignName}" (ROAS: ${roas.toFixed(2)}, Spend: $${totalSpend.toFixed(2)}, End: ${matchingContract.end_date})`);
            return;
          }
          ...
          if (recentImpressions === 0) {
            debugCounts.noRecentImpressions++;
            console.log(`🔍 ROAS Debug: ❌ No recent impressions for "${campaignName}" (ROAS: ${roas.toFixed(2)}, Spend: $${totalSpend.toFixed(2)}, 3 days ago: ${threeDaysAgo.toISOString()}, Latest data: ${sortedRows[sortedRows.length - 1].DATE})`);
            return;
          }

          debugCounts.passed++;
          console.log(`🔍 ROAS Debug: ✅ PASSED "${campaignName}" (ROAS: ${roas.toFixed(2)}, Spend: $${totalSpend.toFixed(2)}, Days left: ${daysLeft}, Recent impressions: ${recentImpressions})`);
          ...
          debugCounts.roasAbove2++;
        } else if (totalSpend <= 100) {
          debugCounts.spendBelow100++;
        }
      } else {
        debugCounts.noSpend++;
      }
    ...
    console.log('🔍 ROAS Debug: Summary of filtering:');
    console.log('  Total campaigns:', debugCounts.total);
    console.log('  ❌ Test campaigns:', debugCounts.testCampaigns);
    console.log('  ❌ No spend:', debugCounts.noSpend);
    console.log('  ❌ ROAS >= 2.0:', debugCounts.roasAbove2);
    console.log('  ❌ Spend < $100:', debugCounts.spendBelow100);
    console.log('  ❌ No matching contract:', debugCounts.noContract);
    console.log('  ❌ Days left = 0:', debugCounts.daysLeftZero);
    console.log('  ❌ No recent impressions:', debugCounts.noRecentImpressions);
    console.log('  ✅ Passed all checks:', debugCounts.passed);
    console.log('  Final ROAS alerts count:', sections.roas.length);
